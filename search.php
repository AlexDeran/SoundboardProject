...
if(isset($_POST['search']) && !empty($_POST['search'])):
  ...
  $searchsnd = $pdo->prepare("SELECT Nom , Son FROM sound WHERE Nom = :search");
  $searchsnd->bindParam(':search', $search,PDO::PARAM_STR);
  $searchsnd->execute();
  

  if($searchsnd):
				$searchyboi = $searchsnd->fetch(PDO::FETCH_ASSOC);
    else:
        $searchyboi = false;
    endif;
  endif;
		...
		<section>
			<article>
				<div class="container-fluid">
					<div class="row">
						<div class="col">
               <?php foreach ($searchyboi as $searchy):?>
								<div>
									<div class="col"><?=$searchy?>
								</div>
								<audio controls>
									<source src="SBP/<?=$searchy?>" type="audio/mpeg">
								</audio>
							 </div>
							<?php endforeach;?>
						</div>
